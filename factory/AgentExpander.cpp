...
{
    const bool effect_capability =
        capability == "filesystem.write" || capability == "filesystem.read" ||
        capability == "process.execute" || capability == "compiler.execute" ||
        capability == "network.request";

    std::string prompt;
    if (effect_capability)
    {
        prompt = "Genera la especificación de un microagente C++ para Satellite con EFECTOS de sistema. Objetivo: " + goal + ". Capacidad requerida: " + capability + ". Puede crear/editar archivos (sandbox_write_file), leerlos (sandbox_read_file), ejecutar comandos (sandbox_run_process) y hacer requests GET (sandbox_http_get). Los paths son relativos al proyecto. Responde SOLO con JSON: {\"name\": \"...\", \"description\": \"...\", \"input_schema\": {...}, \"output_schema\": {...}, \"implementation_code\": \"...\", \"test_cases\": [{\"input\": {...}, \"expected\": {...}}]}. El implementation_code incluye #include \"core/agent/AgentSandbox.h\" y usa el sandbox (req.sandbox). El JSON del implementation_code debe tener los saltos de línea como \\n escapados.";
    }
    else
    {
